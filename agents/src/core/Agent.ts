...
export class EarnlabAgent {
  ...
  async initialize(storageCid?: string): Promise<void> { if (storageCid) await this.memory.load(storageCid, this.config.ownerAddress); }
  async run(): Promise<ExecutionResult> {
    ...
    try {
      ...
      const newCid = await this.memory.persist(this.config.inftTokenId, this.config.ownerAddress);
