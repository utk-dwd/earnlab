...
contract AgentRegistry is Ownable {
    ...
    constructor(address inftAddress) Ownable(msg.sender) {
        inftContract = IERC7857(inftAddress);
    }
    ...
    function registerAgent(
        ...
        // Verify caller owns the iNFT or is authorized executor
        require(
            inftContract.ownerOf(inftTokenId) == msg.sender ||
            inftContract.isAuthorizedExecutor(inftTokenId, msg.sender),
            "Not iNFT owner or executor"
        );
    ...
    function triggerExecution(uint256 agentId, bytes calldata executionParams)
        external returns (bytes32 executionId)
    {
        Agent storage agent = _agents[agentId];
        require(agent.status == AgentStatus.Active, "Agent not active");
        require(
            msg.sender == agent.owner || msg.sender == agent.strategyExecutor,
            "Not authorized"
        );
